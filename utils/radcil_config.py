...
实验入口的默认行为，只为阶段 2 新主流程提供显式、可审计的参数源。
"""
...
class RadcilStage2Config:
    """阶段 2 WiSig 主流程的最小锁定配置。
    ...
    - `closedset_epochs`、`cil_head_warmup_epochs`、`cil_joint_epochs`：主流程
      训练轮数。它们保持为显式配置，后续可在正式三种子版本中统一调整。
    """
...
def get_stage2_wisig_main_config(seed: int | None = None) -> RadcilStage2Config:
    """返回阶段 2 WiSig 主流程配置。

    `seed` 仅用于后续扩展正式多种子运行；默认值代表当前单种子主流程。
    """
    base = RadcilStage2Config()
    if seed is None or seed == base.seed:
        return base
    return RadcilStage2Config(seed=int(seed))
