"""ADS-B 10 -> 20 -> 30 -> 40 staged open-set incremental loader.

The downloaded corpus contains two disjoint label spaces:

* ``X_train_90Class.npy`` / ``Y_train_90Class.npy``: pool of 90 initially
  available aircraft identities.
...
offset after selecting the ten known aircraft.  This loader maps selected known
aircraft to 0..9 and novel aircraft to 10..39.
"""
...
def load_adsb_10known_3round(
    ...
    """Load a leakage-conscious 10-known + 3x10-unknown ADS-B protocol."""
    if (initial_known_classes, round_size, num_rounds) != (10, 10, 3):
        raise ValueError("This loader is specialized for ADS-B 10 -> 20 -> 30 -> 40")
    ...
    if len(available_known) < 10:
        raise ValueError("The 90-class pool contains fewer than ten classes")
    ...
    selection_rng = np.random.default_rng(known_selection_seed)
    selected_known_original = sorted(
        selection_rng.choice(available_known, size=initial_known_classes, replace=False).astype(int).tolist()
    )
    ...
    result.update(
        {
            "dataset": "ADS-B 90-known-pool + 30-novel-pool",
            "data_root": str(root),
...
if __name__ == "__main__":
    ...
    splits = load_adsb_10known_3round(
        args.data_root,
