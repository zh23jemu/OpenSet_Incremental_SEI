"""Strict ADS-B 90 -> 100 -> 110 -> 120 staged incremental loader.
...
Leakage boundary
----------------
The 90-class file has no capture/session identifiers.  To avoid a randomized
sample-level train/test mixture, each class is split in its stored order: the
first 70% form the development pool and the final 30% form the held-out base
evaluation pool.  The development pool is subsequently split 6/7 : 1/7 by
the training code, yielding 60% train / 10% validation / 30% evaluation.

For the 30 novel identities, the corpus-provided train and test files are kept
strictly disjoint.  Hidden labels from the test file are never returned to any
discovery, calibration, pseudo-labelling, replay, or training path.
"""
...
def _class_stratified_known_split(
    ...
    for new_label, original_label in enumerate(selected_original_classes):
        ...
        # Deliberately do not shuffle.  The source corpus does not expose
        # capture/session identifiers; a stored-order block is a stricter
        # proxy than mixing potentially adjacent windows across both sides.
        ...
        split_rows.append(
            {
                ...
                "train": int(len(tr)),
                "eval": int(len(ev)),
                "strategy": "stored_order_block_70_30",
            }
        )
    ...
    return (
        ...
        {
            "per_class": split_rows,
            "index_overlap": 0,
            "strategy": "stored_order_block_70_30",
            "capture_metadata_available": False,
            "seed_used_for_known_split": False,
        },
    )
...
def load_adsb_90known_3round(
    ...
    """Load the strict dataset-native 90-known + 3x10-novel protocol."""
    ...
    result: Dict[str, object] = {
        "day1_known_train": {
            ...
            "tx_range": "0-89",
            "split": f"stored-order development block {train_ratio:.0%}",
        },
        "day1_initial_eval": {
            "X": x_known_eval,
            "y": y_known_eval,
            ...
            "tx_range": "0-89",
            "split": f"stored-order held-out block {1.0 - train_ratio:.0%}",
        },
    }
    ...
    result.update(
        {
            ...
            "protocol_version": "adsb_90known_strict_orderblock_v1",
            "novel_partition_policy": "provided train file for discovery/training; provided test file for evaluation only",
