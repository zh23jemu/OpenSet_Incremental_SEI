...
56. Stage 51 已实现 LoRa 新类吸收 seed31 小矩阵：主入口新增默认关闭的新类 imprint scale/bias 和 head warmup CE boost；Slurm 入口为 `slurm/stage51_lora_new_class_absorption_seed31.sbatch`，报告器为 `tools/stage51_lora_new_class_absorption_report.py`。下一步推送后在独立 worktree 提交短任务，只过 New/Overall/Forgetting 门槛才扩三种子。
...
23. Stage 50 已完成；old:new batch ratio 不是突破口。Stage 51 已切到新类原型吸收/分类头结构，当前要先跑 seed31 小矩阵，不继续调相邻采样配比。
