...
- Stage75 已新增训练期旧类跨天监督适配：每轮 discovery 前读取当前日期旧设备 IQ_1-7 标注样本，直接优化 LoRa backbone 的旧类 CE、Day1 known CE、旧类特征对齐和 discovery teacher 锚定；严格不读取 IQ_8-10。独立 worktree Job `46537156` 因缺少临时生成的 Stage46 s28 NPZ 失败，算法未开始；现已把同口径 s28 重建步骤内置到入口，待重提。
