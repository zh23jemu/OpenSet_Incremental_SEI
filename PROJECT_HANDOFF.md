...
44. Stage 42 Job `45891194` 已完成 LoRa 全局 discovery 实例对比预训练：Chirp 对照 R3 Overall/Old/New/Forgetting=`0.2581/0.1988/0.4952/0.2738`，Global SSL=`0.1610/0.1333/0.2714/0.2000`；Overall 和 New 大幅下降，归档为负消融，不扩三种子。结果已通过 `stage42-results-45891194` 分支同步。
...
60. Stage 55 LoRa raw s28 物理域预训练 seed7 Job `46166372` 已完成并同步小型结果：R3 Overall/Old/New/Forgetting=`0.2805/0.2345/0.4643/0.2310`，相对 Stage48 seed7 Old `+0.0125`，但 Overall `-0.0005`、New `-0.0524`、Forgetting `+0.0583`；未过门槛，归档为负消融，不扩三种子。
...
65. Stage 60 LoRa 累积伪标签全量重训 seed7 Job `46281697` 已完成并同步小型结果：R3 Overall/Old/New/Forgetting=`0.1029/0.1262/0.0095/0.1917`，显著低于 Stage48；说明当前 discovery 伪标签噪声不能直接作为全量监督数据重训分类器，不扩三种子。
66. Stage 61 LoRa oracle discovery 上界诊断 Job `46308084` 已完成：R3 Overall/Old/New/Forgetting=`0.2986/0.2220/0.6048/0.2095`，完美聚类明显抬 New，但 Overall 仍只有约 30%，Old/Forgetting 仍差，判定为 `NOT_DISCOVERY_ONLY`。
...
77. Stage 72 LoRa 更多旧类 logreg 校准 Job `46484861` 已完成：IQ_1-4 与 IQ_1-5 的 R3 Overall/Old/New=`0.4438/0.4167/0.5524`、`0.4524/0.4274/0.5524`。相对 Stage71 继续正向，但 5 条 transmission 仍未到 50；下一步 Stage73 继续验证 IQ_1-6/IQ_1-7。
...
- Stage76 Job `46630851` 已完成：旧类跨天 IQ_1-7 标注直接加入每轮 CIL 后，R3 Overall/Old/New/Forgetting=`0.2886/0.2601/0.4024/0.1631`。它抬高了 Old，但将 New 从 Stage48 seed7 的 `0.5167` 压到 `0.4024`；按旧新类平衡门槛归档为负消融，不扩三种子。下一步只做旧类专家和伪新类分类头分离的结构性验证。
- Stage77 Job `46634865` 已完成并通过 Git 同步：训练后分离旧类跨天专家与伪新类增量头，R3 Overall/Old/New/Forgetting=`0.3043/0.3387/0.1667/0.1798`，旧路由比例 `0.5705`。Overall/Old 虽提高，但 New 严重下降，未通过门槛，不扩三种子；这条线暂时收口。
- 当前下一步：不再继续调旧类路由比例、校准阈值或旧类监督强度。若继续攻 LoRa，只开新的结构方向，例如训练期可验证的旧/新来源判别信号或更强跨天表征；否则保持 Stage48 为正式 strict 候选，并把 LoRa 低分作为跨体制局限如实汇报。
