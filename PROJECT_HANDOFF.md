...
- 更新时间：2026-07-29
- 当前阶段：阶段 7 训练期跨天表征验证已启动；阶段 6 的 GPCC、增量双视图一致性和 discovery 簇可靠性加权均未解决最终低分。新候选 Job `45049066` 正在等待资源，尚无结果
...
- 已新增阶段 7 训练期跨天 CORAL-style discovery/replay 特征分布对齐入口：新增默认关闭的 `--radcil_domain_alignment_weight`，并配套 stage7 计划、报告器、Slurm 入口和合成 smoke test；Job `45049066` 已提交，当前因账户并发限制 pending。
...
4. 等待 Job `45049066`；若训练期跨天分布对齐未通过 IQ_7/held-out 双门槛，则 LoRa 停止继续加机制，转入最终局限报告。
