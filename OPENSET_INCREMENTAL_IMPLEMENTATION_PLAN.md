...
Stage 39 将 Stage 38 的局部正信号扩展为 discovery 未标注物理预训练。该候选仍只替换初始 LoRa Chirp checkpoint，但额外把协议允许的 Day2-4 IQ_1-7 discovery/enrollment 样本用于无标签物理描述符回归；不使用未知设备标签，不读取 Day1 IQ_8-10 或 Day2-4 IQ_8-10 held-out evaluation。seed7 门槛为超过 Stage 38 seed7 的 R3 Overall `0.2705`，Old 不低于 `0.2031`，New 不低于 `0.4800`；未通过则直接归档，不扩三种子。
