...
- ADS-B 三种子 R3 均只发现 6–7 个簇，正式结果仍受后期发现欠聚类限制。
...
- [ ] 推送本地提交并运行自适应密度三种子与 ADS-B strict baseline Slurm 作业，按实证结果生成最终表。
...
2026-07-28 已实现无标签轮次自适应密度：以 0.03 为锚点，仅当 0.02 的新增簇不超过 2、silhouette 至少提升 0.04、置信度/CV/小簇比例未明显恶化且候选间 ARI 达标时才切换；所有候选与拒绝原因写入独立审计 CSV。与此同时修复了 ADS-B 开启 baseline 时未初始化冻结特征银行和发现信息的执行错误，并准备两套三种子 Slurm 入口。当前代码已本地提交但未获明确推送授权，尚无新 Slurm 实证。
...
| ADS-B R2/R3 发现欠聚类                  | 三种子 R3 仅发现 6–7/10 类，限制 New Acc 和 Overall                                             | 固定 Long-RADCIL 后端，以 0.03 为锚点运行严格无标签轮次自适应密度三种子验证                |
...
1. 保留 ADS-B 正式 ratio 0.03；获得明确推送授权后运行已实现的严格无标签自适应密度三种子验证。
2. 运行 ADS-B strict baseline 三种子作业，生成共享发现后端、Deep-HDBSCAN 端到端和发现前端消融总表。
3. 自适应密度若不稳定则归档为负消融并停止继续调固定密度参数；完成 baseline 表后进入 LoRa 跨体制验证。
