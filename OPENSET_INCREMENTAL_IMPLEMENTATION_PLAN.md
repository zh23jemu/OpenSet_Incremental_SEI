...
| Stage 75 LoRa 训练期旧类跨天监督适配 | 修复后 Job `46537623` 已提交，当前排队 | 每轮 discovery 前读取当前日期旧设备 IQ_1-7 标注样本，入口会从 Stage44 原始 IQ 自动重建同口径 s28 输入，再联合优化旧类 CE、Day1 known CE、旧类特征对齐和 discovery teacher 锚定；严格排除 IQ_8-10 held-out eval |
