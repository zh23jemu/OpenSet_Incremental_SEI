...
Stage 38 已转入 LoRa-specific 物理域表征预训练。该候选只替换初始 LoRa Chirp checkpoint，不改变 strict split、GPCC recording-consensus、cross-day、LoRa SSL、top0.80 注册或 RADCIL 后端；checkpoint 训练只使用 Day1 IQ_1-6 训练样本与 IQ_7 验证样本，不读取 Day1 IQ_8-10 或后续天 held-out eval。训练目标为 CE + SupCon + 物理描述符回归，描述符包含相位步进均值/方差、幅度均值/方差、频谱质心和频谱带宽。seed7 Job `45651503` 已完成，R3 Overall/Old/New/Forgetting 从 Stage 37 baseline `0.2552/0.1988/0.4810/0.2786` 提升到 `0.2705/0.2131/0.5000/0.2429`，通过 seed7 门槛。下一步使用新增三种子入口验证 seed7/13/31；只有三种子均值稳定超过 Stage 27 Chirp 当前候选且 New 不塌缩，才锁定为 LoRa 新正式候选。
