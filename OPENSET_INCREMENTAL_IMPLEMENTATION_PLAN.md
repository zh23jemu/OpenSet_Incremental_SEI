...
| Stage 52 LoRa dechirped s28 | 已完成，未通过 | Job `46166237` 中 dechirped s28 R3 Overall/Old/New/Forgetting=`0.0367/0.0137/0.1286/0.0988`，远低于 Stage48 seed7 raw s28+rec065，对闭集和聚类均有破坏，不扩三种子 |
...
| Stage 57 LoRa top80 + old-route | 已完成，未通过 | Job `46178133` 中 `top080_oldroute` 将 Old 提到 `0.2470`，但 Overall `0.2762`、New `0.3929`、Forgetting `0.2940`，相对 Stage48 New `-0.1238` 且遗忘恶化 `+0.1214`，不扩三种子 |
| Stage 58 LoRa 双模型后验路由 | 已完成，未通过 | Job `46268271` 中 best 仍为 base，R3 Overall/Old/New/Forgetting=`0.2814/0.2137/0.5524/0.2214`；双模型 `new_conf_delta_0p00/0p05/0p10` 的 Overall 为 `0.2790/0.2781/0.2786`，均低于 base，Old/Forgetting 变差，不扩三种子 |
| Stage 59 LoRa 物理描述符原型链路 | 已完成，未通过 | Job `46274803` 中 `kmeans` R3 Overall/Old/New/Forgetting=`0.0510/0.0565/0.0286/0.1321`，`kmeans_rec065` 为 `0.0533/0.0589/0.0310/0.1345`；纯物理统计特征远低于 Stage48，不扩三种子 |
| Stage 60 LoRa 累积伪标签重训 | 已完成，未通过 | Job `46281697` R3 Overall/Old/New/Forgetting=`0.1029/0.1262/0.0095/0.1917`；绕开 RADCIL 后端从头重训仍被伪标签噪声拖垮，不扩三种子 |
| Stage 61/62 LoRa oracle 上界诊断 | 已完成，未通过 | Stage61 oracle discovery R3 Overall/Old/New/Forgetting=`0.2986/0.2220/0.6048/0.2095`；Stage62 old-route=`0.2757/0.2524/0.3690/0.2571`，grouped DOI=`0.3000/0.2232/0.6071/0.2750`。完美聚类和现有后端保护也没有打开 Old/New 同时可行的上界 |
| Stage 63 LoRa 增强一致性预训练 | 已完成，弱正但未通过 | Job `46319406` R3 Overall/Old/New/Forgetting=`0.3029/0.2494/0.5167/0.2524`；相对 Stage48 seed7 Overall/Old 提升、New 持平，但遗忘明显变差，不直接扩三种子 |
...
| Stage 69 LoRa 真实旧类 IQ 校准 | 已完成，正向但不足 | Job `46462472` 使用 Day2-4 旧设备 IQ_1 标注校准，不读取 IQ_8-10 held-out 真值；R3 Overall/Old/New 从 `0.2814/0.2137/0.5524` 提到 `0.3429/0.2905/0.5524`。方向有效，但单个 transmission 不足以达到 50%，下一步 Stage70 比较 IQ_1、IQ_1-2、IQ_1-3 三档校准量 |
| Stage 70 LoRa 旧类校准量矩阵 | 已完成，正向但不足 | Job `46479985`：prototype 校准 IQ_1/IQ_1-2/IQ_1-3 的 R3 Overall/Old/New 为 `0.3429/0.2905/0.5524`、`0.3867/0.3452/0.5524`、`0.3990/0.3607/0.5524`；校准量增加有效，但 3 条 transmission 仍不到 50% |
| Stage 71 LoRa 旧类 logreg 校准 | 已完成，正向但不足 | Job `46481693`：logreg 校准 IQ_1/IQ_1-2/IQ_1-3 的 R3 Overall/Old/New 为 `0.3643/0.3173/0.5524`、`0.3957/0.3565/0.5524`、`0.4114/0.3762/0.5524`；超过 Stage70 prototype 并接近旧类重映射 oracle `0.4157/0.3815/0.5524`，但仍不到 50% |
...
| Stage 76 LoRa 旧类跨天校准联合 CIL | 已完成，负消融 | Job `46630851` 完整运行且严格排除 IQ_8-10；R3 Overall/Old/New/Forgetting=`0.2886/0.2601/0.4024/0.1631`。相对 Stage48 seed7 Overall `+0.0076`、Old `+0.0381`、Forgetting `-0.0095`，但 New `-0.1143`，证明直接把额外旧类跨天标注并入 CIL 只会转移旧新类冲突，不扩三种子 |
| Stage 77 LoRa 旧类专家与伪新类头分离 | 已完成，负消融 | Job `46634865`：R3 Overall/Old/New/Forgetting=`0.3043/0.3387/0.1667/0.1798`，旧类明显改善但 New 相对 Stage48 seed7 下降 `0.3500`；门控把约 `57.05%` 样本路由到旧专家，仍严重吞新类，不扩三种子 |
...
| LoRa 新旧类后端权衡                   | Grouped fusion、训练期类中心锚定、old-logit bias、BN 重校准、新类吸收、dechirped 表示、Stage53 recording 一致性、Stage54 recording CE、Stage55 raw s28 物理域预训练、Stage56 refined filter、Stage57 top80+old-route、Stage58 双模型后验路由、Stage59 纯物理描述符原型、Stage60 累积伪标签重训、Stage61 oracle discovery、Stage62 oracle 后端保护、Stage63 增强一致性预训练、Stage64 masked reconstruction、Stage67 旧类 replay SupCon、Stage74 增强 logits 校准、Stage75 前置适配、Stage76 联合 CIL 和 Stage77 旧类专家门控均未通过最终门槛；Stage77 将 Old 提到 `0.3387`，但 New 降至 `0.1667`，确认旧类专家门控仍会吞掉真实新类；Stage 48 原始 I/Q s28 + recording-consensus 的正式结果仍为 Overall `0.2803±0.0148`、Forgetting `0.1802±0.0185` | 当前正式 strict 候选仍保持 Stage 48；不再继续旧类路由、旧类 CE、特征对齐或校准器特征小改。若继续攻 LoRa，需要新的旧/新来源判别信号或更强跨天表征，而不是加大旧类监督强度 |
...
3. LoRa 双视图一致性、簇可靠性加权、训练期跨天特征分布对齐和 Stage 8 代理度量均已归档为负消融；Stage 48 通过原始 I/Q s28 多窗和 recording-consensus 将正式候选提升到 `0.2803±0.0148`，Stage 49-60 证明相邻后端、伪标签过滤、物理描述符和全量重训均不是突破口；Stage61/62 进一步证明完美聚类和现有后端保护也只能到约 `0.30` Overall；Stage63 增强一致性预训练把 seed7 Overall 推到 `0.3029` 但遗忘变差；Stage64 masked reconstruction New 大幅下降；Stage65/66 证明 recording-level 口径也到不了 50%，真正缺口是 R3 旧类跨天保持需要从约 `0.23` 提到约 `0.51`；Stage67 证明 replay-only old SupCon 也不能打开该缺口。Stage68 同日旧类校准 oracle 已打开 50% 上界，Stage69-73 真实 IQ_1-7 旧类 logreg 校准最高到 `0.4695`，Stage74 增强 logits 校准降到 `0.4567`；Stage75 前置监督适配未提升，Stage76 直接联合 CIL 虽将 Old 提到 `0.2601`，但 New 降至 `0.4024`；Stage77 分离旧类专家把 Old 提到 `0.3387`，但 New 降至 `0.1667`。旧类监督/路由线暂时收口，下一步只有在新增更可靠旧/新来源判别信号时才继续，否则按 Stage48 正式候选和 LoRa 跨天局限汇报。
...
Stage 38 已完成 LoRa-specific 物理域表征预训练验证。该候选只替换初始 LoRa Chirp checkpoint，不改变 strict split、GPCC recording-consensus、cross-day、LoRa SSL、top0.80 注册或 RADCIL 后端；checkpoint 训练只使用 Day1 IQ_1-6 训练样本与 IQ_7 验证样本，不读取 Day1 IQ_8-10 或后续天 held-out eval。训练目标为 CE + SupCon + 物理描述符回归，描述符包含相位步进均值/方差、幅度均值/方差、频谱质心和频谱带宽。seed7 Job `45651503` 将 R3 Overall/Old/New/Forgetting 从 Stage 37 baseline `0.2552/0.1988/0.4810/0.2786` 提升到 `0.2705/0.2131/0.5000/0.2429`，说明物理域目标有局部正信号；但三种子 Job `45652089` 的 R3 Overall/Old/New/Forgetting 为 `0.2508±0.0241/0.1909±0.0296/0.4905±0.0411/0.2413±0.0192`，Overall/Old 低于当前 Stage 27 Chirp 候选 `0.2540/0.1952/0.4889/0.2794`。该方向归档为 seed7 正信号但三种子未通过，不采用为正式 LoRa 方案，不继续物理描述符权重小矩阵。
...
5. 对 ADS-B/LoRa 低结果只做诚实解释和阶段性更新：ADS-B 已超过 50% 强对照，LoRa 已从 25.4% 提到 28.0%，但仍不是高准确率结果；LoRa old-logit bias 证明旧类偏置可解释部分问题，Stage 48 证明原始 I/Q 多窗更有效；Stage55 已证明 raw s28 物理域预训练不能在当前最佳口径上继续提高 Overall。
