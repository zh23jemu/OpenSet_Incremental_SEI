...
| LoRa 新旧类后端权衡                   | Grouped fusion、训练期类中心锚定、old-logit bias、BN 重校准、新类吸收、dechirped 表示、Stage53 recording 一致性、Stage54 recording CE、Stage55 raw s28 物理域预训练、Stage56 refined filter、Stage57 top80+old-route、Stage58 双模型后验路由、Stage59 纯物理描述符原型、Stage60 累积伪标签重训、Stage61 oracle discovery、Stage62 oracle 后端保护、Stage63 增强一致性预训练、Stage64 masked reconstruction、Stage67 旧类 replay SupCon、Stage74 增强 logits 校准、Stage75 前置适配和 Stage76 联合 CIL 均未通过最终门槛；Stage76 将 Old 提到 `0.2601`，但 New 降至 `0.4024`，确认额外旧类标签直接进入统一分类头会转移冲突；Stage 48 原始 I/Q s28 + recording-consensus 的正式结果仍为 Overall `0.2803±0.0148`、Forgetting `0.1802±0.0185` | 当前正式 strict 候选仍保持 Stage 48；下一步只验证“旧类跨天专家与伪新类分类头分离”的双专家结构，避免校准梯度直接挤压新类列；不再叠加旧类 CE/特征对齐权重 |
