...
- 状态：实施中；阶段 0–4 已收束；阶段 5 已转向训练期旧类原型锚定，seed7 预注册矩阵待运行
- 版本：1.1
- 创建日期：2026-07-26
- 最近更新：2026-07-28
...
- [ ] 运行原型锚定权重 `0/0.25/1.0` seed7 矩阵；只按 IQ_7 R3 旧类保持率选择候选，再对 held-out 新旧类权衡做事后审计。
...
| LoRa 新旧类后端权衡                     | Job `44578823` R3 New `0.4286`、Forgetting `0.4357` 改善，但 Overall `0.1495`、Old `0.0798` 未达门槛 | 已转向训练期 Teacher replay 类中心锚定；只按 IQ_7 选择，held-out 仅事后审计 |
...
3. 获得明确授权后推送并提交 LoRa 原型锚定 seed7 矩阵；若 IQ_7 或 held-out 权衡门槛失败，停止该方向。
