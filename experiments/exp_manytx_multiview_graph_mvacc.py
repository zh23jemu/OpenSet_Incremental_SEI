# -*- coding: utf-8 -*-
"""
WiSig ManyTx cross-day open-set incremental SEI experiment.

10-known / 3-round protocol:
    A deterministic 40-Tx subset is selected using sample availability only.
    Day 1: mapped Tx 0-9 known training
    Day 2: mapped Tx 10-19 unknown discovery round 1
    Day 3: mapped Tx 20-29 unknown discovery round 2
    Day 4: mapped Tx 30-39 unknown discovery round 3
...
def load_manytx_10known_3round(
    dataset_path,
    ...
    def collect(mapped_labels, day_i, part):
        """Use a fixed seed to create a random sample-level development/evaluation split."""
        xs, ys = [], []
        for mapped_label in mapped_labels:
            ...
            for rx_i in selected_rx_list:
                ...
                rng = np.random.default_rng(int(seed) + 100000 * day_i + 1000 * original_tx + rx_i)
...
def train_closedset_model(
    ...
    train_closedset_with_validation(
        ...
        use_rf_augmentation=use_rf_augmentation,
...
def run_discovery(method, round_name, day_name, true_new_classes, X_round, y_round, Z_round, args, full_method=False, iarc_method=False):
    feats = build_round_features(X_round, Z_round, args)
    ...
    if mv_acc_method:
        # Use the same density micro-cluster prior as the graph-fusion ablation.
        # This keeps the comparison fair and avoids a dataset-specific hidden
        # cluster-count prior in the final method.
    ...
    # Keep raw per-method diagnostic plots optional; the paper-facing output is
    # saved once per MV-ACC round with both known and newly discovered classes.
    ...
    if args.enable_visualization and args.save_detailed_visualizations:
        save_discovery_visualizations(
            method=method, round_name=round_name, day_name=day_name,
            features=discovery_feat, y_true=y_round, cluster_labels=labels_for_metrics,
            accepted_ids=enrolled_ids, args=args,
    ...
    info = {
        ...
        "discovery_features": discovery_feat,
    }
...
def calibrate_mvacc_on_known_day1(X_cal, y_cal, Z_cal, args):
    ...
    if not ratios or not thresholds:
...
def main():
    ...
    parser.add_argument("--dataset_path", type=str, default=r"C:\Users\123\Downloads\ManyTx.pkl\ManyTx.pkl")
    parser.add_argument("--save_dir", type=str, default="./results/manytx_rx2_10known_3round_mvacc")
    parser.add_argument("--protocol_path", type=str, default=None, help="Frozen JSON file containing the selected 40 original Tx indices.")
    ...
    parser.add_argument("--projection_hidden_dim", type=int, default=128, help="Hidden dimension of the training-only SupCon projection head.")
    parser.add_argument("--projection_dim", type=int, default=64, help="Output dimension of the training-only SupCon projection head.")
    parser.add_argument("--disable_rf_augmentation", action="store_true", help="Disable conservative training-only IQ augmentation.")
    parser.add_argument("--disable_cflcg", action="store_true", help="Disable Day1-validation CF-LCG classical-feature selection.")
    parser.add_argument("--cflcg_gate_threshold", type=float, default=0.80, help="Open the RF gate only when Day1-validation local purity reaches this value.")
    ...
    parser.add_argument(
        ...
        type=str,
        default="2",
        help="Fixed ManyTx receiver index. Default 2 uses the third receiver.",
    )
    parser.add_argument("--eq_index", type=int, default=0, help="ManyTx equalization index.")
    parser.add_argument("--min_samples_per_tx", type=int, default=40, help="Minimum samples required for every selected Tx on every day.")
    ...
    parser.add_argument("--save_detailed_visualizations", action="store_true", help="Also save legacy per-method unknown-only plots (disabled by default).")
    ...
    if len(selected_rx_list) != 1:
        raise ValueError("ManyTx experiment requires exactly one receiver index.")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    if args.protocol_path is None:
        args.protocol_path = os.path.join(args.save_dir, "manytx_protocol.json")
    if args.checkpoint is None:
        args.checkpoint = os.path.join(args.save_dir, f"closedset_manytx_rx{selected_rx_list[0]}_day1_10known.pth")

    print("\n========== WiSig ManyTx 10-Known 3-Round Incremental Experiment ==========")
    print(f"Dataset: {args.dataset_path}")
    print(f"Save dir: {args.save_dir}")
    print(f"Device: {device}")
    print("Protocol: 10 known + 10 unknown R1 + 10 unknown R2 + 10 unknown R3")
    print(f"Receiver setting: selected_rx_list={selected_rx_list} | mode=single-Rx")
    print(f"Equalization index: {args.eq_index} | minimum samples/Tx/day: {args.min_samples_per_tx}")
    print(f"Frozen protocol: {args.protocol_path}")
    ...
    splits = load_manytx_10known_3round(
    ...
    checkpoint_metadata = {
        ...
        "eq_index": int(args.eq_index),
        "selected_original_tx": list(splits["selected_original_tx"]),
    ...
    with open(os.path.join(args.save_dir, "split_protocol.json"), "w", encoding="utf-8") as f:
        json.dump({"split_level": "sample-level stratified random", "seed": int(args.seed),
                   ...
                   "development_ratio": float(args.development_ratio)}, f, indent=2)
    ...
    if not args.disable_cflcg:
        cflcg_selection, cflcg_extractor = select_classic_feature_view(
            ...
            gate_threshold=args.cflcg_gate_threshold,
        )
        extract_rf_features_batch = cflcg_extractor
        save_selection(cflcg_selection, args.save_dir)
        print(f"[CF-LCG] view={cflcg_selection['output_feature_group']} | "
              f"purity={cflcg_selection['best_purity']:.3f} | gate_open={cflcg_selection['rf_gate_open']}")
        if not cflcg_selection["rf_gate_open"]:
            args.mvacc_lambda_deep, args.mvacc_lambda_rf, args.mvacc_lambda_graph = 0.6, 0.0, 0.4
    else:
        print("[CF-LCG] disabled: using the fixed 24-D RF view.")
    ...
    for method, is_full, is_iarc in methods:
        ...
        if method in {"Deep only", "RF only", "MV-ACC"} and args.enable_visualization:
            feature_key = {"Deep only": "deep", "RF only": "rf", "MV-ACC": "graph"}[method]
            known_features = build_round_features(X_train, Z_train, args)[feature_key]
            for visual_round, visual_info in enumerate(round_infos, start=1):
                seen_features = [known_features]
                seen_true = [y_train]
                seen_operational = [y_train]
                for j in range(visual_round):
                    seen_features.append(round_infos[j]["discovery_features"])
                    seen_true.append(round_data[j]["y"])
                    seen_operational.append(args.initial_known_classes + round_infos[j]["labels"])
                if method == "MV-ACC":
                    save_seen_class_visualizations(
                        np.concatenate(seen_features), np.concatenate(seen_true),
                        np.concatenate(seen_operational), args.save_dir, f"R{visual_round}", args.seed,
                    )
                save_paper_method_visualizations(
                    method, f"R{visual_round}",
                    np.concatenate(seen_features[:-1]), np.concatenate(seen_operational[:-1]),
                    round_infos[visual_round - 1]["discovery_features"],
                    args.initial_known_classes + round_infos[visual_round - 1]["labels"],
                    args.save_dir, args.seed, round_infos[visual_round - 1].get("metrics"),
                )
        ...
        for i, info in enumerate(round_infos, start=1):
            ...
            incremental_rows.append(evaluate_stage(
                ...
                eval_X_name=f"{eval_key}_30pct_{seen_classes}_seen",
