...
def load_wisig_crossday_10known_3round(
    ...
    def collect(tx_indices, day_i, part):
        """Use a fixed seed to create a stratified random sample-level 70/30 split."""
        xs, ys = [], []
        for tx_i in tx_indices:
            for rx_i in selected_rx_list:
                arr = np.asarray(data[tx_i][rx_i][day_i][eq_index])  # (N, 256, 2)
                # A separate deterministic permutation per Tx/Rx/day prevents
                # original recording order from deciding train versus eval.
                rng = np.random.default_rng(int(seed) + 100000 * day_i + 1000 * tx_i + rx_i)
...
def main():
    ...
    with open(os.path.join(args.save_dir, "split_protocol.json"), "w", encoding="utf-8") as f:
        json.dump({
            "split_level": "sample-level stratified random",
            "seed": int(args.seed),
    ...
    proto_banks = {
        mode: build_proto_feature_bank(
            Z_train, X_train, [rd["Z"] for rd in round_data], [rd["X"] for rd in round_data],
            eval_Z_dict, eval_X_dict, args, cflcg_mode=mode,
        ) for mode in ("none", "global", "local")
    }
    proto_bank = proto_banks["none"]
    ...
    methods = [
        ...
        ("No CF-LCG (MV-ACC)", False, False),
        ("Global CF-LCG (MV-ACC)", False, False),
        ("MV-ACC", False, False),
    ]
    ...
    for method, is_full, is_iarc in methods:
        ...
        if method in {"Deep only", "RF only", "MV-ACC"} and args.enable_visualization:
            ...
            known_graph = build_round_features(X_train, Z_train, args, cflcg_mode="local")[feature_key]
            for visual_round in range(1, len(round_infos) + 1):
                ...
                previous_features = fs[:-1]
                previous_operational = os_[:-1]
                save_paper_method_visualizations(
                    method, f"R{visual_round}",
                    np.concatenate(previous_features), np.concatenate(previous_operational),
        ...
        proto_bank = proto_banks[cflcg_mode_for_method(method)]
        proto_type = round_infos[0]["proto_feat_type"]
        n_subprototypes = int(args.mvacc_prototypes_per_class) if "MV-ACC" in method else 1
