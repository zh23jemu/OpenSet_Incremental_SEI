...
from exp_wisig_mvacc_cil_strict import (
...
def load_manytx_10known_3round(
    ...
    def collect(mapped_labels, day_i, part):
        """Order-block development/evaluation proxy; stored samples are not shuffled."""
        xs, ys = [], []
...
def main():
    ...
    with open(os.path.join(args.save_dir, "split_protocol.json"), "w", encoding="utf-8") as f:
        json.dump({"split_level": "stored-order block split (capture metadata unavailable)", "seed": int(args.seed),
