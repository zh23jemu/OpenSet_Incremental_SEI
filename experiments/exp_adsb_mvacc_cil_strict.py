"""Strict ADS-B MV-ACC-CIL experiment.

Dataset-native class protocol: 90 -> 100 -> 110 -> 120 identities.

The base 90-class source file is split by a per-class stored-order block into
70% development and 30% held-out evaluation.  The development portion is
further divided into 60% backbone training and 10% validation.  The 30 novel
identities use the corpus-provided training file for discovery/incremental
training and the corpus-provided test file for evaluation only.

No ADS-B evaluation sample, evaluation label, or evaluation metric is used for
feature selection, clustering calibration, pseudo-label generation, classifier
initialisation, replay, distillation, early stopping, or hyperparameter choice.
...
The hidden discovery labels are used only after clustering to build a frozen
Hungarian ID mapping for offline scoring.  They never affect clustering or
training.  Real deployment outputs internal pseudo-device IDs.
"""
...
def _sample_digest(sample):
    """Stable exact-content digest used only by the split-integrity audit."""
...
def audit_exact_overlap(left_x, right_x, left_name, right_name):
    """Fail closed when exact normalized IQ samples cross a train/test boundary."""
    left_hashes = {_sample_digest(sample) for sample in np.asarray(left_x)}
    right_hashes = {_sample_digest(sample) for sample in np.asarray(right_x)}
    overlap = left_hashes.intersection(right_hashes)
    row = {
        "left": str(left_name),
        "right": str(right_name),
        "left_samples": int(len(left_x)),
        "right_samples": int(len(right_x)),
        "exact_content_overlap": int(len(overlap)),
        "digest": "BLAKE2b-128 over normalized float32 IQ",
    }
    if overlap:
        raise RuntimeError(
            f"Strict split audit failed: {len(overlap)} exact IQ samples occur in "
...
def main():
    ...
    parser.add_argument("--data_root", type=str, default=r"C:\Users\123\Downloads\Dataset\Dataset")
    parser.add_argument("--save_dir", type=str, default="./results/adsb_90known_mvacc_cil_strict_seed31")
    ...
    if args.initial_known_classes != 90 or args.round_size != 10 or args.num_rounds != 3:
        raise ValueError("This script is specialized for ADS-B 90 -> 100 -> 110 -> 120.")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    if args.checkpoint is None:
        args.checkpoint = os.path.join(args.save_dir, "closedset_adsb_90known_strict.pth")

    print("\n========== Strict ADS-B 90-Known 3-Round MV-ACC-CIL Experiment ==========")
    ...
    print(f"Base-class protocol: {args.development_ratio * 6 / 7:.0%} backbone train + {args.development_ratio / 7:.0%} validation + {1.0 - args.development_ratio:.0%} stored-order held-out evaluation")
    print("Novel-class protocol: provided train file for discovery/training; provided test file for evaluation only")
    ...
    splits = load_adsb_90known_3round(
        ...
        initial_known_classes=args.initial_known_classes,
        round_size=args.round_size,
    ...
    # Hard integrity gate.  This audit is diagnostic only and cannot influence
    # model parameters.  Any exact train/evaluation IQ overlap aborts the run.
    split_audit_rows = [
        audit_exact_overlap(
            X_train,
            eval_data["eval_initial"]["X"],
            "base_90_development",
            "base_90_heldout_evaluation",
        )
    ]
    for i, rd in enumerate(round_data, start=1):
        class_start = args.initial_known_classes + (i - 1) * args.round_size
        class_end = class_start + args.round_size
        eval_mask = (
            (eval_data[f"eval_r{i}"]["y"] >= class_start)
            & (eval_data[f"eval_r{i}"]["y"] < class_end)
        )
        split_audit_rows.append(
            audit_exact_overlap(
                rd["X"],
                eval_data[f"eval_r{i}"]["X"][eval_mask],
                f"novel_R{i}_provided_train",
                f"novel_R{i}_provided_test",
            )
        )
    save_csv(split_audit_rows, os.path.join(args.save_dir, "split_integrity_audit.csv"))

    print("\n[Splits]")
    print(f"Base development 70%: {X_train.shape}, classes={np.unique(y_train).size}, labels={np.min(y_train)}-{np.max(y_train)}")
    ...
    for i, rd in enumerate(round_data, start=1):
        ek = f"eval_r{i}"
        print(f"Novel R{i} train file: {rd['X'].shape}, classes={np.unique(rd['y']).size}, labels={np.min(rd['y'])}-{np.max(rd['y'])}, source={rd['day']}")
        print(f"After R{i} held-out eval: {eval_data[ek]['X'].shape}, classes={np.unique(eval_data[ek]['y']).size}, labels={np.min(eval_data[ek]['y'])}-{np.max(eval_data[ek]['y'])}, source={eval_data[ek]['day']}")
    print("[Integrity] exact IQ overlap across every development/test boundary: 0")

    protocol_manifest = {
        "protocol_version": "adsb_90known_strict_mvacc_cil_v1",
        "class_schedule": [90, 100, 110, 120],
        "base_partition": "per-class stored-order 70% development / 30% held-out; development becomes 60% train / 10% validation",
        "novel_partition": "provided X_train_30Class only for discovery/training; provided X_test_30Class only for evaluation",
        ...
        "true_labels_forbidden_from": [
            "feature extraction",
            "CF-LCG local gating on unknown rounds",
            "HDBSCAN",
            "noise reassignment",
            "cluster merge/split",
            "pseudo-label CE",
        ...
        "seed": int(args.seed),
        "locked_cli_configuration": vars(args),
        "known_split_audit": splits.get("known_split_audit", {}),
    }
    with open(os.path.join(args.save_dir, "strict_protocol_manifest.json"), "w", encoding="utf-8") as f:
    ...
    checkpoint_metadata = {
        ...
        "sample_split_protocol": "base_stored_order_60_10_30_and_novel_provided_train_test_v1",
        "seed": int(args.seed),
    ...
    with open(os.path.join(args.save_dir, "split_protocol.json"), "w", encoding="utf-8") as f:
        json.dump({
            "split_level": "base stored-order block; novel corpus-provided train/test (capture metadata unavailable)",
            "seed": int(args.seed),
            "base_backbone_train_samples": int(len(day1_training_subset)),
            "base_validation_samples": int(len(day1_validation_subset)),
            "base_heldout_evaluation_samples": int(len(eval_data["eval_initial"]["y"])),
            "development_ratio": float(args.development_ratio),
