# -*- coding: utf-8 -*-
"""
WiSig ManyRx cross-day/open-receiver incremental SEI experiment.

10-known / 3-round protocol:
    Day 1: mapped Device 0-9 known training
    Day 2: mapped Device 10-14 unknown discovery round 1
    Day 3: mapped Device 15-19 unknown discovery round 2
    Day 4: mapped Device 20-24 unknown discovery round 3

Evaluation:
    IQ transmissions 1-7 are used only for training/discovery/enrollment.
    Independent IQ transmissions 8-10 are used only for cumulative evaluation.
...
from utils.classic_feature_gating import select_classic_feature_view, save_selection
from utils.incremental_visualization import save_seen_class_visualizations
...
def load_manyrx_4known_3round(dataset_path, protocol="crossrx", source_manifest=None):
    """Load a frozen 4 -> 6 -> 8 -> 10 ManyRx compact protocol."""
    if protocol not in {"fixedrx", "crossrx"}:
        raise ValueError(f"Unsupported ManyRx protocol: {protocol}")
    required = [
        "day1_known_train",
        "day1_initial_eval",
        "day2_unknown_round1",
    ...
    with np.load(dataset_path, allow_pickle=False) as data:
        splits = {}
        day_by_stage = {
            "day1_known_train": "Day1",
            "day1_initial_eval": "Day1",
            "day2_unknown_round1": "Day2",
            "day2_eval_after_r1": "Day2",
            "day3_unknown_round2": "Day3",
            "day3_eval_after_r2": "Day3",
            "day4_unknown_round3": "Day4",
            "day4_eval_after_r3": "Day4",
        }
        for stage in required:
            prefix = f"{protocol}_{stage}"
            x_key, y_key = f"{prefix}_X", f"{prefix}_y"
            if any(key not in data.files for key in [x_key, y_key]):
                raise KeyError(f"Missing ManyRx compact arrays for stage: {prefix}")
            X = np.asarray(data[x_key], dtype=np.float32)
            y = np.asarray(data[y_key], dtype=np.int64)
            rx_index = np.asarray(data[f"{prefix}_rx_index"], dtype=np.int16)
            day_index = np.asarray(data[f"{prefix}_day_index"], dtype=np.int16)
            sample_index = np.asarray(data[f"{prefix}_sample_index"], dtype=np.int16)
            if X.ndim != 3 or X.shape[1] != 2:
                raise ValueError(f"Expected {stage} X shape [N,2,L], got {X.shape}")
            if any(len(array) != len(X) for array in [y, rx_index, day_index, sample_index]):
                raise ValueError(f"Length mismatch in stage {stage}")
            splits[stage] = {
                "X": X,
                "y": y,
                "rx_index": rx_index,
                ...
                "recording_id": None,
                "day": day_by_stage[stage],
                "split": protocol,
            }
        physical_device_order = list(range(10))
        device_names = np.asarray(data["tx_names"]).astype(str).tolist()

    expected_classes = {
        "day1_known_train": (4, 0, 3),
        "day1_initial_eval": (4, 0, 3),
        "day2_unknown_round1": (2, 4, 5),
        "day2_eval_after_r1": (6, 0, 5),
        "day3_unknown_round2": (2, 6, 7),
        "day3_eval_after_r2": (8, 0, 7),
        "day4_unknown_round3": (2, 8, 9),
        "day4_eval_after_r3": (10, 0, 9),
    }
    for stage, (n_classes, lo, hi) in expected_classes.items():
        labels = np.unique(splits[stage]["y"])
        if len(labels) != n_classes or int(labels.min()) != lo or int(labels.max()) != hi:
            raise ValueError(f"Unexpected labels for {stage}: {labels.tolist()}")

    source_info = {}
    if source_manifest:
        with open(source_manifest, "r", encoding="utf-8") as f:
            source_info = json.load(f)
    splits["physical_device_order"] = physical_device_order
    splits["device_names"] = device_names
    splits["source_manifest"] = source_info
    splits["train_ratio"] = None
    return splits

    """
    Load WiSig ManyTx and create a deterministic 10-known + 3-round split.
    ...
    The 40 original Tx indices are selected only by cross-day sample
    availability, never by clustering or evaluation performance. They are then
    remapped to labels 0..39 and persisted in a JSON protocol file.
    """
    ...
    if len(selected_rx_list) != 1:
        raise ValueError("ManyTx protocol requires exactly one fixed receiver.")
    if eq_index < 0 or eq_index >= len(obj.get("equalized_list", [0])):
        raise ValueError(f"Invalid eq_index={eq_index}")

    def sample_count(original_tx, day_i):
        total = 0
        for rx_i in selected_rx_list:
            arr = np.asarray(data[original_tx][rx_i][day_i][eq_index])
    ...
    if protocol_path and os.path.exists(protocol_path):
        ...
        selected_original_tx = [int(x) for x in protocol["selected_original_tx_indices"]]
        if len(selected_original_tx) != 40 or len(set(selected_original_tx)) != 40:
            raise ValueError("Protocol must contain exactly 40 unique Tx indices.")
    else:
        eligible = []
        for original_tx in range(n_tx):
            counts = [sample_count(original_tx, day_i) for day_i in range(4)]
            if min(counts) >= int(min_samples_per_tx):
                eligible.append((original_tx, counts))
        if len(eligible) < 40:
            raise ValueError(
                f"Only {len(eligible)} Tx have >= {min_samples_per_tx} samples "
                "on every one of the four days."
            )
        # Fixed original-index order prevents performance-based cherry-picking.
        selected_original_tx = [x[0] for x in eligible[:40]]
        protocol = {
            "dataset_path": os.path.abspath(dataset_path),
            "receiver_indices": list(selected_rx_list),
            "receiver_names": [str(rx_list[i]) for i in selected_rx_list],
            "eq_index": int(eq_index),
            "train_ratio": float(train_ratio),
            "min_samples_per_tx": int(min_samples_per_tx),
            "selected_original_tx_indices": selected_original_tx,
            "mapped_label_to_original_tx": {
                str(i): int(tx) for i, tx in enumerate(selected_original_tx)
            },
            "per_tx_day_counts": {
                str(tx): [sample_count(tx, day_i) for day_i in range(4)]
                for tx in selected_original_tx
            },
        }
        if protocol_path:
            ensure_dir(os.path.dirname(os.path.abspath(protocol_path)))
            with open(protocol_path, "w", encoding="utf-8") as f:
                json.dump(protocol, f, ensure_ascii=False, indent=2)

    for original_tx in selected_original_tx:
        counts = [sample_count(original_tx, day_i) for day_i in range(4)]
        if min(counts) < int(min_samples_per_tx):
            raise ValueError(
                f"Protocol Tx {original_tx} no longer satisfies min_samples_per_tx: {counts}"
            )

    def collect(mapped_labels, day_i, part):
        """part='train' uses first train_ratio samples; part='eval' uses held-out remainder."""
        xs, ys = [], []
        for mapped_label in mapped_labels:
            original_tx = selected_original_tx[mapped_label]
            for rx_i in selected_rx_list:
                arr = np.asarray(data[original_tx][rx_i][day_i][eq_index])  # (N, 256, 2)
                ...
                ys.append(np.full(arr.shape[0], mapped_label, dtype=np.int64))

        if len(xs) == 0:
            raise ValueError(f"No samples collected for mapped_labels={list(mapped_labels)}, day_i={day_i}, part={part}")
    ...
    return {
        ...
        "selected_original_tx": selected_original_tx,
        "protocol": protocol,
    }
...
def train_closedset_model(train_set, num_classes, feat_dim, epochs, batch_size, lr, device, save_path, use_supcon=False, supcon_weight=0.1, supcon_temperature=0.2, checkpoint_metadata=None, seed=7, validation_fraction=1.0 / 7.0, use_rf_augmentation=True, projection_hidden_dim=128, projection_dim=64, validation_groups=None):
    ...
    train_closedset_with_validation(
        ...
        projection_dim=projection_dim, validation_groups=validation_groups,
    )
...
def build_bounded_edge_adaptive_fusion(G_deep, G_rf, alpha_min=0.65, alpha_max=0.95, eps=1e-8):
    """
...
def graph_embedding(z_deep, z_rf, alpha, top_k, graph_dim, seed, adaptive_fusion=False, alpha_min=0.65, alpha_max=0.95):
    Gd = cosine_graph(z_deep, top_k)
    Gr = rbf_graph(z_rf, top_k)

    if adaptive_fusion:
        G, fusion_stats = build_bounded_edge_adaptive_fusion(
            Gd, Gr, alpha_min=alpha_min, alpha_max=alpha_max
        )
    else:
...
def build_round_features(X_round, Z_round, args):
    rf = extract_rf_features_batch(X_round)
...
def aggregate_recording_features(X_round, Z_round, recording_ids, args):
    """Build one multi-view graph node per observable recording boundary.

    Every symbol is retained: clustering is performed on recording-level means,
    then the resulting pseudo-label is propagated back to all member symbols.
    The grouping key contains no transmitter identity or ground-truth class.
    """
    recording_ids = np.asarray(recording_ids)
    unique_ids = np.unique(recording_ids)
    rf_instance = extract_rf_features_batch(X_round)
    deep_group = np.stack(
        [np.mean(Z_round[recording_ids == rid], axis=0) for rid in unique_ids], axis=0
    )
    rf_group = np.stack(
        [np.mean(rf_instance[recording_ids == rid], axis=0) for rid in unique_ids], axis=0
    )
    z_deep = clean_scale(deep_group)
    z_rf = clean_scale(rf_group)
    z_graph, fusion_stats = graph_embedding(
        z_deep,
        z_rf,
        args.alpha,
        min(args.top_k, max(len(unique_ids) - 1, 1)),
        args.graph_dim,
        args.seed,
        adaptive_fusion=args.adaptive_fusion,
        alpha_min=args.alpha_min,
    ...
    return {
        ...
        "hybrid": np.concatenate([z_deep, z_rf], axis=1).astype(np.float32),
        "fusion_stats": fusion_stats,
        "unique_recording_ids": unique_ids,
    }


def propagate_recording_labels(recording_labels, unique_recording_ids, recording_ids):
    label_by_id = {
        int(rid): int(label)
        for rid, label in zip(unique_recording_ids, recording_labels)
    }
    return np.asarray([label_by_id[int(rid)] for rid in recording_ids], dtype=np.int64)
...
def aggregate_features_by_recording(features, labels, recording_ids):
    """Average features inside each capture and keep its propagated label."""
    recording_ids = np.asarray(recording_ids)
    unique_ids = np.unique(recording_ids)
    group_features, group_labels = [], []
    for rid in unique_ids:
        idx = np.flatnonzero(recording_ids == rid)
        group_features.append(np.mean(features[idx], axis=0))
        values, counts = np.unique(np.asarray(labels)[idx], return_counts=True)
        group_labels.append(values[int(np.argmax(counts))])
    return (
        l2norm(np.stack(group_features, axis=0)),
...
def build_proto_feature_bank(Z_train, X_train, round_Z_list, round_X_list, eval_Z_dict, eval_X_dict):
    """
    ...
    """
    rf_train = extract_rf_features_batch(X_train)
    round_rf_list = [extract_rf_features_batch(x) for x in round_X_list]
    eval_rf_dict = {k: extract_rf_features_batch(x) for k, x in eval_X_dict.items()}
...
def evaluate_stage(
    ...
    if recording_ids is not None:
        ids = np.asarray(recording_ids)[seen_mask]
        unique_ids = np.unique(ids)
        group_features = np.stack(
            [np.mean(eval_features[seen_mask][ids == rid], axis=0) for rid in unique_ids],
            axis=0,
        )
        group_pred_raw = predict_proto(
            group_features,
            prototypes,
            proto_labels,
        ...
        pred_by_id = {
            int(rid): pseudo_to_true.get(int(pred), int(pred))
            for rid, pred in zip(unique_ids, group_pred_raw)
        }
        y_pred = np.asarray([pred_by_id[int(rid)] for rid in ids], dtype=np.int64)
    else:
...
def run_incremental_learning_baselines(discovery_round_infos, proto_bank, y_train, round_data, eval_data, eval_y_dict, args, device, method_prefix="", comparison_label="Shared discovery", feat_type="hybrid"):
    ...
    def eval_initial_linear(method, model, class_ids):
        ...
        row = _evaluate_raw_predictions(
            ...
            eval_X_name="day1_eval_transmission8to10",
    ...
    row_icarl_init = _evaluate_raw_predictions(mname("iCaRL"), "Initial", eval_data["eval_initial"]["day"], args.initial_known_classes, "day1_eval_transmission8to10", eval_y_dict["eval_initial"], pred_icarl_init, {}, "-", "-", "-", args.initial_known_classes, args.round_size, None)
    ...
    row_tpcil_init = _evaluate_raw_predictions(mname("TPCIL-style"), "Initial", eval_data["eval_initial"]["day"], args.initial_known_classes, "day1_eval_transmission8to10", eval_y_dict["eval_initial"], pred_tpcil_init, {}, "-", "-", "-", args.initial_known_classes, args.round_size, None)
    ...
    row_doi_init = _evaluate_raw_predictions(mname("DOI-style"), "Initial", eval_data["eval_initial"]["day"], args.initial_known_classes, "day1_eval_transmission8to10", eval_y_dict["eval_initial"], pred_doi_init, {}, "-", "-", "-", args.initial_known_classes, args.round_size, None)
    ...
    for i, chunk in enumerate(pseudo_chunks, start=1):
        ...
        stage = f"After R{i}"
        eval_name = f"{eval_key}_transmission8to10_{seen_classes}_seen"
...
def plot_core_method_visualization_panels(args):
    """Create paper-friendly UMAP/t-SNE panels for all reported methods."""
    ...
    method_dirs = [
        ...
        ("No-drop ablation", "no_drop_consolidation_ablation"),
        ("MV-ACC", "mv_acc"),
    ]
    out_dir = os.path.join(args.save_dir, "visualizations", "core_comparison")
    ensure_dir(out_dir)

    projections = ["umap", "tsne"] if args.visualization_method == "both" else [args.visualization_method]
    for r in ["r1", "r2", "r3"]:
      for projection in projections:
        csv_name = "embedding_2d.csv" if projection == "umap" else f"embedding_2d_{projection}.csv"
        csv_paths = [
            os.path.join(args.save_dir, "visualizations", d, r, csv_name)
            for _, d in method_dirs
        ]
        if not all(os.path.exists(path) for path in csv_paths):
            continue
        ...
        for label_col, suffix, title_suffix in [
            ...
            fig, axes = plt.subplots(1, len(method_dirs), figsize=(24, 4.5))
            for ax, (method_name, _), path in zip(axes, method_dirs, csv_paths):
                ...
                ax.set_xticks([])
                ax.set_yticks([])
            fig.suptitle(f"{r.upper()} method comparison ({projection.upper()}): {title_suffix}")
            plt.tight_layout()
            save_path = os.path.join(out_dir, f"{r}_{suffix}_all_methods_{projection}.png")
...
def save_discovery_visualizations(method, round_name, day_name, features, y_true, cluster_labels, accepted_ids, args, full_method=False):
    ...
    for projection_method in projection_methods:
        print(f"[Visualize] {method} {round_name}: projecting {features.shape[0]} samples by {projection_method}")
        ...
        plot_labeled_embedding(
            ...
            save_path=os.path.join(out_dir, f"true_labels{suffix}.png"),
            noise_label=-999999,
        )
        plot_labeled_embedding(
            points,
            ...
            save_path=os.path.join(out_dir, f"clusters{suffix}.png"),
            noise_label=-1,
        )

        if full_method:
            plot_reliability_embedding(
                points,
                cluster_labels,
                accepted_ids,
                title=f"{method} {round_name} {day_name}: assignment status ({projection_method.upper()})",
                save_path=os.path.join(out_dir, f"assignment_status{suffix}.png"),
            )
        ...
        embedding_df.to_csv(
            os.path.join(out_dir, f"embedding_2d{suffix}.csv"),
            index=False,
            encoding="utf-8-sig",
        )
        # Keep UMAP at the legacy path so the comparison-panel builder can run.
        if args.visualization_method == "both" and projection_method == "umap":
            embedding_df.to_csv(
                os.path.join(out_dir, "embedding_2d.csv"),
                index=False,
                encoding="utf-8-sig",
            )
...
def run_discovery(method, round_name, day_name, true_new_classes, X_round, y_round, Z_round, args, full_method=False, iarc_method=False, recording_ids=None):
    instance_feats = build_round_features(X_round, Z_round, args)
    use_recording_graph = (
        method == "MV-ACC"
        and bool(args.mvacc_recording_graph)
        and recording_ids is not None
    )
    if use_recording_graph:
        feats = aggregate_recording_features(X_round, Z_round, recording_ids, args)
        node_true = np.asarray([
            y_round[np.flatnonzero(np.asarray(recording_ids) == rid)[0]]
            for rid in feats["unique_recording_ids"]
        ])
    else:
    ...
    mv_acc_method = base_method == "MV-ACC"
    if mv_acc_method:
        # Scale the density prior with the current stream size.  Earlier
        # versions exposed ``mvacc_min_cluster_ratio`` but accidentally kept
        # using the fixed baseline min_cluster_size, making Day1 calibration a
        # no-op and encouraging transmission-level micro-clusters on LoRa.
        discovery_min_cluster_size = max(
            2,
            int(np.ceil(float(args.mvacc_min_cluster_ratio) * len(discovery_feat))),
        )
        discovery_min_samples = int(
            args.mvacc_recording_min_samples if use_recording_graph
            else args.mvacc_min_samples
        )
    else:
    ...
    raw_metrics = clustering_metrics(node_true, raw_labels)
    ...
    if mv_acc_method:
        ...
        #   2) high-confidence micro-clusters are consolidated first;
        #   3) every remaining noise sample is assigned by three-view consensus.
        ...
        iarc_stats = {
            ...
            **split_stats,
            ...
            "MVACC Min Cluster Ratio": float(discovery_min_cluster_size / max(len(discovery_feat), 1)),
    ...
    node_labels_for_metrics = labels_for_metrics
    if use_recording_graph:
        labels_for_metrics = propagate_recording_labels(
            node_labels_for_metrics,
            feats["unique_recording_ids"],
            recording_ids,
        )
    ...
    cluster_row = {
        ...
        "Discovery Nodes": int(len(discovery_feat)),
        "Grouping": "recording-boundary" if use_recording_graph else "instance",
    ...
    # Save each method × round independently for fair visual comparison.
    if args.enable_visualization:
        save_discovery_visualizations(
        method=method,
        round_name=round_name,
        day_name=day_name,
        features=instance_feats["graph"] if use_recording_graph else discovery_feat,
        y_true=y_round,
    ...
    info["discovery_features"] = instance_feats["graph"] if use_recording_graph else discovery_feat
    return cluster_row, info


def calibrate_mvacc_on_known_day1(X_cal, y_cal, Z_cal, args, recording_ids=None):
    """Calibrate density and consolidation on held-out Day1 known classes only."""
    prior_ratio = float(args.mvacc_min_cluster_ratio)
    prior_threshold = float(args.mvacc_merge_threshold)
    ratios = [float(x) for x in args.mvacc_calibration_ratios.split(",") if x.strip()]
    thresholds = [float(x) for x in args.mvacc_calibration_thresholds.split(",") if x.strip()]
    recording_min_samples = [
        int(x) for x in args.mvacc_calibration_recording_min_samples.split(",") if x.strip()
    ] if args.mvacc_recording_graph and recording_ids is not None else [int(args.mvacc_min_samples)]
    if not ratios or not thresholds or not recording_min_samples:
        raise ValueError("MV-ACC calibration candidate lists must not be empty.")

    rows = []
    for ratio in ratios:
        for threshold in thresholds:
          for recording_min_sample in recording_min_samples:
            candidate = copy.copy(args)
            candidate.enable_visualization = False
            candidate.mvacc_min_cluster_ratio = float(ratio)
            candidate.mvacc_merge_threshold = float(threshold)
            candidate.mvacc_recording_min_samples = int(recording_min_sample)
            row, _ = run_discovery(
                "MV-ACC",
                "Calibration",
                "Day1 held-out known",
            ...
            row["Candidate Min Cluster Ratio"] = float(ratio)
            row["Candidate Merge Threshold"] = float(threshold)
            row["Candidate Recording Min Samples"] = int(recording_min_sample)
            rows.append(row)

    calibration_df = pd.DataFrame(rows)
    calibration_df["Calibration Score"] = (
        calibration_df["NMI"].astype(float)
        + calibration_df["ARI"].astype(float)
        + calibration_df["Hungarian Acc"].astype(float)
        - 0.25 * calibration_df["Cluster Count Error"].astype(float)
    )
    calibration_df = calibration_df.sort_values(
        ["Cluster Count Error", "Calibration Score", "Candidate Min Cluster Ratio", "Candidate Merge Threshold"],
        ascending=[True, False, True, True],
    ).reset_index(drop=True)
    calibration_path = os.path.join(args.save_dir, "mvacc_day1_known_calibration.csv")
    calibration_df.to_csv(calibration_path, index=False, encoding="utf-8-sig")

    informative = any(
        calibration_df[col].astype(float).nunique() > 1
        for col in ["Final Cluster Count", "NMI", "ARI", "Hungarian Acc"]
    )
    ...
    if informative:
        ...
        args.mvacc_recording_min_samples = int(best["Candidate Recording Min Samples"])
    else:
        # Do not let an arbitrary tie silently change a pre-registered setting.
        args.mvacc_min_cluster_ratio = prior_ratio
        args.mvacc_merge_threshold = prior_threshold
    print("\n[MV-ACC calibration | Day1 held-out known labels only]")
    print(
        calibration_df[[
            "Candidate Min Cluster Ratio", "Candidate Merge Threshold",
            "Candidate Recording Min Samples",
            "Final Cluster Count", "Cluster Count Error", "Purity", "NMI", "ARI",
            "Hungarian Acc", "Calibration Score",
        ]].to_string(index=False, float_format=lambda x: f"{x:.6f}")
    )
    print(
        f"[MV-ACC calibration] frozen ratio={args.mvacc_min_cluster_ratio}, "
        f"merge_threshold={args.mvacc_merge_threshold}, informative={informative} | saved={calibration_path}"
    )


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", type=str, default=os.path.join(PROJECT_ROOT, "datasets", "manyrx_compact", "manyrx_4known_3round_fixed_cross_rx.npz"))
    parser.add_argument("--save_dir", type=str, default="./results/manyrx_4known_3round_mvacc")
    parser.add_argument("--source_manifest", type=str, default=os.path.join(PROJECT_ROOT, "datasets", "manyrx_compact", "manyrx_protocol_manifest.json"))
    parser.add_argument("--protocol", choices=["fixedrx", "crossrx"], default="crossrx")
    ...
    parser.add_argument("--closedset_val_ratio", type=float, default=1.0 / 7.0, help="Validation fraction inside the Day1 training portion.")
    ...
    parser.add_argument("--initial_known_classes", type=int, default=4)
    parser.add_argument("--round_size", type=int, default=2)
    ...
    parser.add_argument(
        "--mvacc_recording_graph",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Aggregate symbols into observable recording-boundary graph nodes, then propagate labels to all symbols.",
    )
    parser.add_argument("--mvacc_recording_min_samples", type=int, default=2, help="HDBSCAN min_samples for recording-level MV-ACC graph nodes.")
    parser.add_argument("--mvacc_merge_rounds", type=int, default=12, help="Maximum mutually-nearest consolidation rounds for MV-ACC.")
    parser.add_argument("--mvacc_merge_threshold", type=float, default=0.78, help="Frozen multi-view prototype merge threshold for MV-ACC.")
    ...
    parser.add_argument("--disable_mvacc_calibration", action="store_true", help="Use supplied MV-ACC parameters without Day1 known-class calibration.")
    parser.add_argument("--mvacc_calibration_ratios", type=str, default="0.03,0.045,0.06,0.08", help="Comma-separated density-ratio candidates calibrated on held-out Day1 known classes.")
    parser.add_argument("--mvacc_calibration_thresholds", type=str, default="0.74,0.78,0.82,0.86", help="Comma-separated consolidation thresholds calibrated on held-out Day1 known classes.")
    parser.add_argument("--mvacc_calibration_recording_min_samples", type=str, default="1,2,3", help="Recording-graph HDBSCAN min_samples candidates calibrated on held-out Day1 known classes.")
    ...
    parser.add_argument("--visualization_method", type=str, default="both", choices=["tsne", "pca", "umap", "both"], help="Visualization projection method; 'both' saves UMAP and t-SNE in one run.")
    ...
    if args.initial_known_classes != 4 or args.round_size != 2 or args.num_rounds != 3:
        raise ValueError("This script is specialized for ManyRx: 4 known + 3 rounds of 2 classes.")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    if args.checkpoint is None:
        args.checkpoint = os.path.join(args.save_dir, f"closedset_manyrx_{args.protocol}_day1_4known.pth")

    print("\n========== WiSig ManyRx 4-Known 3-Round Incremental Experiment ==========")
    print(f"Dataset: {args.dataset_path}")
    print(f"Save dir: {args.save_dir}")
    print(f"Device: {device}")
    print("Class protocol: 4 known + 2 unknown R1 + 2 unknown R2 + 2 unknown R3")
    print(f"Receiver protocol: {args.protocol}")
    print("fixedrx: Rx 1-1 first 70% discovery / last 30% evaluation")
    print("crossrx: Rx 1-1 discovery / Rx 1-19 evaluation")
    ...
    splits = load_manyrx_4known_3round(
    ...
    print("\n[Splits]")
    print(f"Day1 known discovery/train: {X_train.shape}, classes={np.unique(y_train).size}, labels={np.min(y_train)}-{np.max(y_train)}")
    print(f"Initial independent eval: {eval_data['eval_initial']['X'].shape}, classes={np.unique(eval_data['eval_initial']['y']).size}, labels={np.min(eval_data['eval_initial']['y'])}-{np.max(eval_data['eval_initial']['y'])}, day={eval_data['eval_initial']['day']}")
    for i, rd in enumerate(round_data, start=1):
        ek = f"eval_r{i}"
        print(f"Unknown R{i} discovery:  {rd['X'].shape}, classes={np.unique(rd['y']).size}, labels={np.min(rd['y'])}-{np.max(rd['y'])}, day={rd['day']}")
        print(f"After R{i} cumulative eval: {eval_data[ek]['X'].shape}, classes={np.unique(eval_data[ek]['y']).size}, labels={np.min(eval_data[ek]['y'])}-{np.max(eval_data[ek]['y'])}, day={eval_data[ek]['day']}")

    train_set = to_dataset(X_train, y_train)
    checkpoint_metadata = {
        "dataset_path": os.path.abspath(args.dataset_path),
        "source_manifest": os.path.abspath(args.source_manifest),
        "physical_device_order": list(splits["physical_device_order"]),
        "known_tx": list(range(args.initial_known_classes)),
        "training_day_index": 0,
        "protocol": args.protocol,
        "training_receiver_index": 0,
        "evaluation_receiver_index": 0 if args.protocol == "fixedrx" else 1,
        "seed": int(args.seed),
        ...
        "group_disjoint_validation": bool(splits["day1_known_train"].get("recording_id") is not None),
    ...
    if args.train_closedset or not os.path.exists(args.checkpoint):
        model = train_closedset_model(
            ...
            validation_groups=splits["day1_known_train"].get("recording_id"),
        )
    else:
    ...
    day1_training_recordings = splits["day1_known_train"].get("recording_id")
    day1_training_subset, day1_validation_subset = stratified_train_validation_split(
        train_set,
        validation_fraction=args.closedset_val_ratio,
        seed=args.seed,
        groups=day1_training_recordings,
    )
    ...
    with open(os.path.join(args.save_dir, "split_protocol.json"), "w", encoding="utf-8") as f:
        json.dump({
            "split_level": "recording-disjoint when recording identifiers are available",
            "protocol": args.protocol,
            "seed": int(args.seed),
    ...
    global extract_rf_features_batch
    if not args.disable_cflcg:
        ...
        extract_rf_features_batch = cflcg_extractor
        ...
        if not cflcg_selection["rf_gate_open"]:
            args.mvacc_lambda_deep, args.mvacc_lambda_rf, args.mvacc_lambda_graph = 0.6, 0.0, 0.4
    ...
    if not args.disable_mvacc_calibration:
        calibrate_mvacc_on_known_day1(
            X_train[day1_validation_indices],
            y_train[day1_validation_indices],
            Z_train[day1_validation_indices],
            args,
            recording_ids=(
                np.asarray(day1_training_recordings)[day1_validation_indices]
                if day1_training_recordings is not None else None
            ),
        )

    print("\n[Build] Prototype feature banks")
    ...
    methods = [
        ("Deep only", False, False),
        ("RF only", False, False),
        ("Graph fusion (raw)", False, False),
        ("No-drop consolidation (ablation)", False, False),
        ("MV-ACC", False, False),
    ]

    clustering_rows = []
    incremental_rows = []
    graph_fusion_round_infos = None
    deep_only_round_infos = None

    # Initial model is common to all methods.
    init_proto, init_labels = make_prototypes(proto_bank["train"]["deep"], y_train)
    initial_row = evaluate_stage(
        method="Initial model",
        stage="Initial",
        eval_day=eval_data["eval_initial"]["day"],
        seen_classes=args.initial_known_classes,
        eval_X_name=f"day1_eval_{args.protocol}",
        eval_features=proto_bank["eval_initial"]["deep"],
        y_eval=eval_y_dict["eval_initial"],
        prototypes=init_proto,
        proto_labels=init_labels,
        pseudo_to_true={},
        true_new="-",
        discovered="-",
        enrolled="-",
    ...
    for method, is_full, is_iarc in methods:
        ...
        if method == "Deep only":
            deep_only_round_infos = copy.deepcopy(round_infos)
        if method == "Graph fusion (raw)":
            graph_fusion_round_infos = copy.deepcopy(round_infos)

        if method == "MV-ACC" and args.enable_visualization:
            known_graph = build_round_features(X_train, Z_train, args)["graph"]
            for visual_round in range(1, len(round_infos) + 1):
                fs, ts, os_ = [known_graph], [y_train], [y_train]
                for j in range(visual_round):
                    fs.append(round_infos[j]["discovery_features"])
                    ts.append(round_data[j]["y"])
                    os_.append(args.initial_known_classes + round_infos[j]["labels"])
                save_seen_class_visualizations(np.concatenate(fs), np.concatenate(ts), np.concatenate(os_),
                                               args.save_dir, f"R{visual_round}", args.seed)

        proto_type = round_infos[0]["proto_feat_type"]
        n_subprototypes = int(args.mvacc_prototypes_per_class) if method == "MV-ACC" else 1
        use_group_classifier = (
            method == "MV-ACC"
            and bool(args.mvacc_recording_graph)
            and splits["day1_known_train"].get("recording_id") is not None
        )
        if use_group_classifier:
            train_proto_features, train_proto_labels, _ = aggregate_features_by_recording(
                proto_bank["train"][proto_type],
                y_train,
                splits["day1_known_train"]["recording_id"],
            )
        else:
            train_proto_features, train_proto_labels = proto_bank["train"][proto_type], y_train
        current_proto, current_labels = make_prototypes(
            train_proto_features,
            train_proto_labels,
            prototypes_per_class=n_subprototypes,
            seed=args.seed,
        )
        pseudo_to_true = {}
        next_label = args.initial_known_classes

        for i, info in enumerate(round_infos, start=1):
            if use_group_classifier:
                enrollment_features, enrollment_labels, _ = aggregate_features_by_recording(
                    proto_bank[f"r{i}"][proto_type],
                    info["labels"],
                    round_data[i - 1]["recording_id"],
                )
            else:
                enrollment_features, enrollment_labels = (
                    proto_bank[f"r{i}"][proto_type], info["labels"]
                )
            r_proto, r_labels, _, cluster_pseudo_pairs, next_label = enroll_new_prototypes(
                enrollment_features,
                enrollment_labels,
                info["enrolled_ids"],
                next_label=next_label,
                reliability_map=info.get("reliability_map", {}),
                default_reliability=1.0,
                prototypes_per_cluster=n_subprototypes,
                seed=args.seed,
            )
            mapping = build_posthoc_pseudo_to_true(
                round_data[i - 1]["y"], info["labels"], cluster_pseudo_pairs
            ...
            if r_proto is not None:
                current_proto = np.concatenate([current_proto, r_proto], axis=0)
                current_labels = np.concatenate([current_labels, r_labels], axis=0)

            seen_classes = args.initial_known_classes + i * args.round_size
            eval_key = f"eval_r{i}"
            incremental_rows.append(evaluate_stage(
                method=method,
                stage=f"After R{i}",
                eval_day=eval_data[eval_key]["day"],
                seen_classes=seen_classes,
                eval_X_name=f"{eval_key}_{args.protocol}_{seen_classes}_seen",
                eval_features=proto_bank[eval_key][proto_type],
                y_eval=eval_y_dict[eval_key],
                prototypes=current_proto,
                proto_labels=current_labels,
                pseudo_to_true=pseudo_to_true,
                true_new=args.round_size,
                discovered=info["discovered_clusters"],
                enrolled=info["enrolled_clusters"],
                initial_known=args.initial_known_classes,
                round_size=args.round_size,
                initial_reference_acc=initial_reference_acc,
                old_class_bonus=args.old_class_bonus,
                recording_ids=(
                    eval_data[eval_key].get("recording_id")
                    if use_group_classifier else None
                ),
            ))
