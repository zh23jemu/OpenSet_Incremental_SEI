...
- Stage88 已完成：每个旧类 2 条同日旧类 support recording 在剩余 held-out 分母上 observed Overall `0.5133`，但按原始 20旧/5新类比例复核为 `0.4976`，距离 50 只差 `0.0030` Old Acc。下一步若继续攻，应优先尝试 support_count=2 下更稳的旧类校准器或 support_count=3 的诊断，而不是回到无 support 小机制。
