#!/usr/bin/env python3
"""汇总 Stage53 LoRa recording 一致性 seed7 矩阵结果。

Stage53 固定 Stage48 的 LoRa raw s28 + recording-consensus 0.65 主配置，
只在 RADCIL 训练期增加当前轮 discovery 的 recording-level logits 一致性。
...
def collect_variant(root: Path, variant: str, job_id: str) -> dict[str, float | str]:
    """收集一个 Stage53 变体的 symbol-level 与 recording-level R3 指标。"""
    save_dir = root / f"lora_s53_{variant}_{job_id}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage53 LoRa recording 一致性 seed7 报告器")
    parser.add_argument("--root", default="results/stage53")
    parser.add_argument("--job-id", required=True)
    parser.add_argument("--variants", nargs="+", default=["base", "rec_w0p10", "rec_w0p30"])
    ...
    lines = [
        f"# Stage53 LoRa Recording 一致性 Seed7 报告（Job {args.job_id}）",
        "",
        "## 结论",
        "",
    ...
    Path(args.summary_json).write_text(
        json.dumps(
            {
                "schema_version": "stage53_lora_recording_consistency_summary_v1",
