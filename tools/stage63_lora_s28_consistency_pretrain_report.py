#!/usr/bin/env python3
"""汇总 Stage63 LoRa raw s28 增强一致性预训练 seed7 结果。

Stage63 只替换初始 LoRa Chirp checkpoint：在 Stage55 物理描述符辅助训练基础上，
额外加入同一样本两种增强视图的 logit/feature 一致性约束。报告器只读取训练
summary 和 held-out CSV，不参与训练或调参。
"""
...
def _metric(row: pd.Series, name: str) -> float:
    """从结果行读取一个浮点指标，并给出清晰错误。"""
    if name not in row:
        raise KeyError(f"Missing metric column {name!r}; available={list(row.index)}")
    return float(row[name])


def collect(root: Path, job_id: str) -> dict[str, float | str]:
    """收集 Stage63 seed7 的 symbol-level 与 recording-level R3 指标。"""
    save_dir = root / f"lora_s28_consistency_seed7_{job_id}"
    symbol = _r3_row(save_dir / "incremental_results.csv")
    record: dict[str, float | str] = {
        "variant": "s28_physical_view_consistency_pretrain",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage63 LoRa raw s28 consistency pretrain seed7 报告器")
    parser.add_argument("--root", default="results/stage63")
    ...
    checkpoint_summary_path = root / f"stage63_lora_s28_consistency_checkpoint_summary_{job_id}.json"
    checkpoint_summary = json.loads(checkpoint_summary_path.read_text(encoding="utf-8"))
    gate = passes_gate(record)

    lines = [
        f"# Stage63 LoRa raw s28 增强一致性预训练 Seed7 报告（Job {job_id}）",
        "",
        "## 结论",
        "",
        ...
        f"- Stage63 R3 Overall/Old/New/Forgetting = "
    ...
    Path(args.summary_json).write_text(
        json.dumps(
            {
                "schema_version": "stage63_lora_s28_consistency_pretrain_summary_v1",
                "job_id": job_id,
